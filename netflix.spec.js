...
describe('Testing Netflix Log In Page', () => {
    ...
    it('Test Netflix Login page', () => {
        ...
        cy.get('.login-signup-now').contains('Sign up now').click({force: true});
    })
    ...
    it('User account not found', () => {
      ...
      it('verify user id id contains not valid password', () => {
        cy.visit('https://www.netflix.com/in/');
        cy.contains('Sign In').click();
  
        cy.get('#id_userLoginId').type('nancy@gmail.com', {force: true})
        
        cy.get('#id_password').type('nancy@gmail.com', {force: true});
